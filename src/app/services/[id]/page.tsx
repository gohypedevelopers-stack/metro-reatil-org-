...
const SUBCATEGORY_DATA: Record<string, {
  ...
  "electro-mechanical": {
    ...
    gallery: ["https://images.unsplash.com/photo-1621905251189-08b45d6a269e?auto=format&fit=crop&q=80", "https://images.unsplash.com/photo-1581094794329-auto=format&fit=crop&q=80", "https://images.unsplash.com/photo-1584622650111-993a426fbf0a?auto=format&fit=crop&q=80"]
  },
  "lighting-design": {
...
export default function SubcategoryDetailPage() {
  ...
  return (
    <div className="bg-white pt-20">
      ...
      {/* UNIQUE SUBCATEGORY HERO */}
      <section className="relative min-h-[45vh] flex items-center bg-brand-dark py-20">
        ...
        <div className="relative z-10 max-w-[1600px] mx-auto px-6 md:px-12 w-full flex flex-col md:flex-row gap-12 items-center">
          <div className="md:w-1/2">
            <span className="text-brand-gold text-[10px] md:text-xs font-bold uppercase tracking-[0.5em] mb-4 block flex items-center gap-2">
              <PenTool size={14} /> Specialized Capability
            </span>
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-serif text-white mb-6 uppercase tracking-tight" style={{ fontFamily: 'var(--font-cinzel), serif' }}>
              {data.title}
            </h1>
            <p className="text-neutral-300 text-lg md:text-xl font-light italic border-l-2 border-brand-gold pl-5" style={{ fontFamily: 'var(--font-playfair), serif' }}>
              {data.tagline}
            </p>
          </div>
          
          <div className="md:w-1/2 bg-black/40 backdrop-blur-md p-8 md:p-12 border border-white/10 rounded-sm">
            <h3 className="text-white text-lg font-bold uppercase tracking-widest mb-6">Service Overview</h3>
            <div className="space-y-4 mb-8">
              {data.desc.map((paragraph, idx) => (
                <p key={idx} className="text-neutral-300 text-sm leading-relaxed font-light">
                  {paragraph}
                </p>
              ))}
            </div>
            <a href="/contact" className="inline-flex items-center gap-3 text-brand-dark bg-brand-gold px-6 py-3 text-[10px] font-bold uppercase tracking-widest hover:bg-white transition-colors">
              Request Technical Consultation <ChevronRight size={14} />
            </a>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-[1600px] mx-auto px-6 md:px-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {data.process.map((p, i) => (
              <div key={i} className="relative p-8 border border-neutral-100 bg-neutral-50/50 hover:bg-white hover:shadow-xl transition-all duration-500 group">
                <div className="absolute top-0 right-0 p-6 text-6xl font-serif text-neutral-100 group-hover:text-brand-gold/10 transition-colors" style={{ fontFamily: 'var(--font-cinzel), serif' }}>
                  {i + 1}
                </div>
                <div className="relative z-10">
                  <h3 className="text-lg font-bold text-brand-dark uppercase tracking-widest mb-4 flex items-center gap-3">
      ...
      <section className="py-20 md:py-28 bg-neutral-900">
        <div className="max-w-[1600px] mx-auto px-6 md:px-12">
          <div className="mb-16 md:w-2/3">
            <h2 className="text-3xl md:text-5xl font-serif text-white uppercase tracking-tight mb-4" style={{ fontFamily: 'var(--font-cinzel), serif' }}>
              Featured {data.title} Work
            </h2>
            <p className="text-neutral-400 font-light text-lg">Examine the precision and quality of our completed installations.</p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {data.gallery.map((img, i) => (
              <div key={i} className="relative aspect-[4/5] overflow-hidden group rounded-sm border border-neutral-800">
                <img
                  src={img}
                  alt={`Project ${i + 1}`}
                ...
                <div className="absolute bottom-6 left-6 right-6">
                  <span className="bg-white/90 backdrop-blur-sm px-4 py-2 text-[9px] font-bold uppercase tracking-widest text-brand-dark shadow-sm">
