...
const PROCESS_PHASES = [
  ...
  },
  {
    ...
    img: "https://images.unsplash.com/photo-1581094216060-ee22621dd758?q=80&w=600&auto=format&fit=crop",
  ...
  },
  {
    ...
    img: "https://images.unsplash.com/photo-1604014237800-1c9102c219da?q=80&w=600&auto=format&fit=crop",
    ...
    <section className="py-24 bg-brand-dark text-white relative overflow-hidden">
      ...
      <div className="max-w-[1600px] mx-auto px-6 md:px-12 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-start">
          ...
          <div className="lg:col-span-7 bg-white/[0.02] border border-white/5 p-8 md:p-12 rounded-sm shadow-2xl min-h-[580px] flex flex-col justify-between">
            <AnimatePresence mode="wait">
              ...
              >
                ...
                <div className="relative aspect-[16/8] overflow-hidden rounded-sm bg-neutral-900 border border-white/5 shadow-inner">
                  <img 
                    src={PROCESS_PHASES[activeStep].img} 
                    alt={PROCESS_PHASES[activeStep].title} 
                  ...
                  <div className="absolute bottom-6 left-6 bg-brand-gold text-white p-3.5 rounded-full shadow-lg border border-brand-gold/20 flex items-center justify-center">
                    {PROCESS_PHASES[activeStep].icon}
                  </div>
                </div>
                ...
                <div className="space-y-5">
                  <div className="flex flex-col gap-1">
                    ...
                    <h4 className="text-2xl font-serif text-white uppercase tracking-wider" style={{ fontFamily: 'var(--font-cinzel), serif' }}>
                      {PROCESS_PHASES[activeStep].title}
                    </h4>
                  </div>
                  <p className="text-neutral-400 text-sm font-light leading-relaxed">
                    {PROCESS_PHASES[activeStep].desc}
                  </p>
                </div>
                ...
                <div className="pt-4">
                  ...
                  <ul className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-3.5">
                    {PROCESS_PHASES[activeStep].highlights.map((h, i) => (
                ...
                {/* Structural Stats Panel */}
                <div className="pt-8 border-t border-white/5 grid grid-cols-3 gap-6 text-left">
                  <div>
                    <span className="text-[7.5px] font-bold uppercase tracking-[0.2em] text-neutral-500 block mb-1">Timeline</span>
                    <span className="text-brand-gold font-serif text-sm uppercase tracking-wide font-medium">{PROCESS_PHASES[activeStep].stats.duration}</span>
                  </div>
                  <div>
                    <span className="text-[7.5px] font-bold uppercase tracking-[0.2em] text-neutral-500 block mb-1">Human Resource</span>
                    <span className="text-brand-gold font-serif text-sm uppercase tracking-wide font-medium">{PROCESS_PHASES[activeStep].stats.crew || PROCESS_PHASES[activeStep].stats.artisans || PROCESS_PHASES[activeStep].stats.officers}</span>
                  </div>
                  <div>
                    <span className="text-[7.5px] font-bold uppercase tracking-[0.2em] text-neutral-500 block mb-1">Standard Vetted</span>
                    <span className="text-brand-gold font-serif text-sm uppercase tracking-wide font-medium">{PROCESS_PHASES[activeStep].stats.precision || PROCESS_PHASES[activeStep].stats.compliance || PROCESS_PHASES[activeStep].stats.inspections || PROCESS_PHASES[activeStep].stats.snags}</span>
                  </div>
                </div>
